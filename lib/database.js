...
async function githubApiRequest(url, method = 'GET', data = {}) {
  try {
    const options = {
      method,
      headers: {
        Authorization: `Basic ${Buffer.from(`${owner}:${octokit}`).toString('base64')}`,
...
async function githubCreateNewFile(filePath, fileName, content) {
  const url = `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}/${fileName}`;
  const data = {
...
config.LOGO = `https://files.catbox.moe/bm2v7m.jpg` 
config.ANTI_BAD = []
